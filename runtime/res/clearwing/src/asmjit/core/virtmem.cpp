...
// Virtual Memory [Switch]
// =====================

#ifdef __SWITCH__
static std::recursive_mutex lock;
static std::map<void *, Jit> mappings;
...
static HardenedRuntimeFlags getHardenedRuntimeFlags() noexcept {
  return HardenedRuntimeFlags::kEnabled;
}

Error releaseDualMapping(DualMapping* dm, size_t size) noexcept {
  std::lock_guard guard{lock};
  auto it = mappings.find(dm->rw);
  if (it == mappings.end())
    return kErrorInvalidAddress;
  Result rc = jitClose(&it->second);
  mappings.erase(it);
  return kErrorOk;
}
...
Error alloc(void** p, size_t size, MemoryFlags flags) noexcept {
  return kErrorFeatureNotEnabled;
}
...
void protectJitMemory(ProtectJitAccess access) noexcept {
  ...
  for (auto &entry : mappings) {
    if (access == ProtectJitAccess::kReadWrite)
      jitTransitionToWritable(&entry.second);
    else
      jitTransitionToExecutable(&entry.second);
  }
#else
