...
public class RobustJavaMethodParser implements JavaMethodParser {
	...
	static {
		...
		plainOldDataTypes.put("bool", ArgumentType.Boolean);
	...
	@Override
	public ArrayList<JavaSegment> parse (String classFile) {
		JavaSymbolSolver solver = new JavaSymbolSolver(new ReflectionTypeSolver());
		CompilationUnit unit = new JavaParser(new ParserConfiguration().setSymbolResolver(solver)).parse(new ByteArrayInputStream(classFile.getBytes())).getResult().get();
	...
	private JavaMethod createMethod (MethodDeclaration method) {
		String className = classStack.peek().getFullyQualifiedName().get();
		String simpleName = classStack.peek().getName().asString();
		boolean nested = classStack.peek().isNestedType();
		String name = method.getNameAsString();
		boolean isStatic = method.isStatic();
		Type returnType = method.getType();
		ArrayList<Parameter> arguments = new ArrayList<>();
		ArrayList<ArgumentType> argumentTypes = new ArrayList<>();

		if (method.getParameters() != null) {
			arguments.addAll(method.getParameters());
			for (Parameter parameter : method.getParameters())
				argumentTypes.add(getArgumentType(parameter));
		}

		return new JavaMethod(className, simpleName, nested, name, isStatic, returnType, null, arguments, argumentTypes, method.getBegin().get().line, method.getEnd().get().line, method.findCompilationUnit().get(), method.toDescriptor());
	}
