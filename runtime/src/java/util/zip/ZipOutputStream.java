...
//        this.zc = ZipCoder.get(charset);
    ...
    public void setComment(String comment) {
        if (comment != null) {
            this.comment = comment.getBytes();// zc.getBytes(comment);
...
//            e.flag |= EFS;
    ...
    private void writeLOC(XEntry xentry) throws IOException {
        ...
        byte[] nameBytes = e.name.getBytes();// zc.getBytes(e.name);
...
//        }
        ...
        byte[] commentBytes;
        if (e.comment != null) {
            commentBytes = e.comment.getBytes();// zc.getBytes(e.comment);
